...
document.getElementById("btn-add").addEventListener("click", () => {
  let addResult = functions.calculatorAdd(
...
document.getElementById("btn-sub").addEventListener("click", () => {
  let subResult = functions.calculatorSub(
    Number(inputOne.value),
    Number(inputTwo.value)
  );
...
document.getElementById("btn-mul").addEventListener("click", () => {
  let mulResult = functions.calculatorMul(
    Number(inputOne.value),
    Number(inputTwo.value)
  );
...
document.getElementById("btn-dvd").addEventListener("click", () => {
  let dvdResult = functions.calculatorDvd(
    Number(inputOne.value),
    Number(inputTwo.value)
  );
...
document.getElementById("btn-tax").addEventListener("click", () => {
  ...
  taxDisplayVal.textContent = `your 2019 taxes would be $${taxResult.toFixed(
    2
  )}`;
  taxInput.value = "";
});
